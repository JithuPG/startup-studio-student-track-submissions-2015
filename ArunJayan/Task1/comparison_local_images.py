...
# python compare.py --local1 path/image --local2 path/image 						  	  #
# eg: python compare.py --local1 TESTT1.png --local2 TESTT2.png 					      #
...
def main():
	ap.add_argument("-i1","--local1",required=True,help="address of image in internet")	#argument 1
	ap.add_argument("-i2","--local2",required=True,help="address of image in internet")	#argument 2
	args = vars(ap.parse_args())
	img1 = cv2.imread(args["local1"])	#read image1 
	img2 = cv2.imread(args["local2"])	#read image2
	...
	if per == 100 :	#if images are exactly same , prints a message .
		print "Exactly same"
	cv2.imshow("source image 1&2",np.hstack([img1,img2]))
