$(function(){
	...
	(function(){
		...
		$('#example-images').click(function(){
			...
			$('#dropzone1').html('<img src="http://127.0.0.1/1.jpg"/>');
			$('#dropzone2').html('<img src="http://127.0.0.1/3.jpg"/>');

			$.when(done, dtwo).done(function(file, file1){
				if (typeof FileReader === 'undefined') {
					resembleControl = resemble('http://127.0.0.1/3.jpg').compareTo('http://127.0.0.1/1.jpg').onComplete(onComplete);
				} else {
					resembleControl = resemble(file).compareTo(file1).onComplete(onComplete);
				}
			});
